...
var Font = class Font {
	constructor(arg) {
		...
		const vectorState = this.vectorState;
		registry.register(this, () => {
			...
			if (vectorState.paintPtr) exports.hb_vector_paint_destroy(vectorState.paintPtr);
		});
	}
	...
	*/
	...
	/**
	* Render a sequence of positioned glyphs to a single SVG document using
	* HarfBuzz's vector paint API. Color glyphs (COLR v0/v1, including
	* gradients and compositing) are emitted with their palette colors;
	* glyphs with no color data fall back to a foreground-colored outline,
	* so plain (monochrome) fonts also produce output.
	*
	* Positions are in font units with the y-axis pointing up (the same
	* convention as shaping output); the returned SVG already accounts for
	* the flip to SVG's y-down space and carries a fitted `viewBox`.
	*
	* @param glyphs Positioned glyphs: `glyph` is a glyph ID (post-shaping),
	*   `x`/`y` the pen position in font units.
	* @param options `palette` selects the CPAL palette index (default 0);
	*   `foreground` is the fallback color as an `hb_color_t` (packed BGRA;
	*   default opaque black).
	* @returns The combined SVG document string, or undefined if nothing was
	*   drawn (e.g. an empty run or glyphs with no ink).
	*/
	glyphsToSvg(glyphs, options = {}) {
		if (!this.vectorState.paintPtr) this.vectorState.paintPtr = exports.hb_vector_paint_create_or_fail(hb_tag("svg "));
		const paint = this.vectorState.paintPtr;
		if (!paint) return void 0;
		exports.hb_vector_paint_clear(paint);
		exports.hb_vector_paint_set_palette(paint, options.palette ?? 0);
		exports.hb_vector_paint_set_foreground(paint, options.foreground ?? 255);
		const prefixPtrs = [];
		for (let i = 0; i < glyphs.length; ++i) {
			const g = glyphs[i];
			const prefix = string_to_ascii_ptr(`g${i}_`);
			prefixPtrs.push(prefix.ptr);
			exports.hb_vector_paint_set_svg_prefix(paint, prefix.ptr);
			exports.hb_vector_paint_set_transform(paint, 1, 0, 0, 1, g.x, g.y);
			exports.hb_vector_paint_glyph(paint, this.ptr, g.glyph, 1);
		}
		const blob = exports.hb_vector_paint_render(paint);
		let svg;
		if (blob) {
			const length = exports.hb_blob_get_length(blob);
			svg = utf8_ptr_to_string(exports.hb_blob_get_data(blob, 0), length);
			exports.hb_vector_paint_recycle_blob(paint, blob);
		}
		for (const p of prefixPtrs) exports.free(p);
		return svg;
	}
	/**
